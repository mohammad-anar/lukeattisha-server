...
const getAll = async (filters: any, options: any) => {
  ...
  const whereConditions: Prisma.FavouriteServiceWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
  ...
  const total = await prisma.favouriteService.count({ where: whereConditions });
