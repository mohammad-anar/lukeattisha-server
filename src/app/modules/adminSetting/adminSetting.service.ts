...
const getAll = async (filters: any, options: any) => {
  ...
  const whereConditions: Prisma.AdminSettingWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
  ...
  const total = await prisma.adminSetting.count({ where: whereConditions });
