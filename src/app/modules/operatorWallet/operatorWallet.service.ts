...
const create = async (payload: any) => {
  const result = await prisma.operatorWallet.create({
    data: payload,
  });
  return result;
};
...
const getAll = async (filters: any, options: any) => {
  ...
  const whereConditions: Prisma.OperatorWalletWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
...
const update = async (id: string, payload: any) => {
  await getById(id);
  const result = await prisma.operatorWallet.update({
    where: { id },
    data: payload,
  });
  return result;
...
const deleteById = async (id: string) => {
  await getById(id);
  const result = await prisma.operatorWallet.delete({
    where: { id },
  });
  return result;
};
