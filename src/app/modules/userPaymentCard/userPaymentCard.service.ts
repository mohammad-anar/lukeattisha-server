...
const getAll = async (filters: any, options: any) => {
  ...
  const whereConditions: Prisma.UserPaymentCardWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
  ...
  const total = await prisma.userPaymentCard.count({ where: whereConditions });
