...
const getAll = async (filters: any, options: any) => {
  ...
  const whereConditions: Prisma.OperatorWalletTransactionWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
  ...
  const total = await prisma.operatorWalletTransaction.count({ where: whereConditions });
