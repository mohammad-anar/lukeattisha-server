...
const getAll = async (filters: any, options: any) => {
  ...
  const whereConditions: Prisma.AdSubscriptionPlanWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
  ...
  const total = await prisma.adSubscriptionPlan.count({ where: whereConditions });
