...
const getAll = async (filters: any, options: any) => {
  ...
  const whereConditions: Prisma.ServiceAddonWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
  ...
  const total = await prisma.serviceAddon.count({ where: whereConditions });
